...
public class ActionAddHealth extends ScriptAction {

	public ActionAddHealth(String name) {
		super("addHealth", name, new TargetType[] { TargetType.ENTITY });
	}
	...
	public void activate(ScriptLine line, ScriptableTargetHolder target, Object... objects) {
		...
		if (target.getEntity() instanceof Damageable)
			((Damageable) target.getEntity()).setHealth(((Damageable) target.getEntity()).getHealth() + h);
	}
	...
	public String[] getParameterTitles() {
		return new String[] { "Health to add (or remove)" };
	}
	@Override
